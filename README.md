...
Метод POST:
Регистрация нового пользователя `/api/register/` --- `username`, `email`, `password`, `first_name`, `last_name`
Вход пользователя `/api/login/` --- `username`, `password`

---

### 2. Получение списков и деталей по ID (ReadOnly)
...
МЕТОД DELETE `/api/cart/items/{id}/` ----- даление товара из корзины
Требуется аутентификация. `id` - ID элемента корзины.
...
МЕТОД GET `/api/orders/{id}/` ------ Получение деталей конкретного заказа
Требуется аутентификация. `id` - ID заказа. Пользователь может просматривать только свои заказы.
